/* slingshot.c generated by valac 0.26.2, the Vala compiler
 * generated from slingshot.vala, do not modify */


#include <glib.h>
#include <glib-object.h>
#include <gtk/gtk.h>
#include <gee.h>
#include <stdlib.h>
#include <string.h>
#include <gdk-pixbuf/gdk-pixbuf.h>
#include <gnome-menus/gmenu-tree.h>
#include <libwnck/libwnck.h>
#include <gdk/gdk.h>
#include <float.h>
#include <math.h>
#include <gio/gdesktopappinfo.h>
#include <gio/gio.h>
#include <stdio.h>
#include <cairo.h>
#include <unique/unique.h>


#define ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW (elementary_widgets_composited_window_get_type ())
#define ELEMENTARY_WIDGETS_COMPOSITED_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, ElementaryWidgetsCompositedWindow))
#define ELEMENTARY_WIDGETS_COMPOSITED_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, ElementaryWidgetsCompositedWindowClass))
#define ELEMENTARY_WIDGETS_IS_COMPOSITED_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW))
#define ELEMENTARY_WIDGETS_IS_COMPOSITED_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW))
#define ELEMENTARY_WIDGETS_COMPOSITED_WINDOW_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, ElementaryWidgetsCompositedWindowClass))

typedef struct _ElementaryWidgetsCompositedWindow ElementaryWidgetsCompositedWindow;
typedef struct _ElementaryWidgetsCompositedWindowClass ElementaryWidgetsCompositedWindowClass;
typedef struct _ElementaryWidgetsCompositedWindowPrivate ElementaryWidgetsCompositedWindowPrivate;

#define TYPE_SLINGSHOT_WINDOW (slingshot_window_get_type ())
#define SLINGSHOT_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), TYPE_SLINGSHOT_WINDOW, SlingshotWindow))
#define SLINGSHOT_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), TYPE_SLINGSHOT_WINDOW, SlingshotWindowClass))
#define IS_SLINGSHOT_WINDOW(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), TYPE_SLINGSHOT_WINDOW))
#define IS_SLINGSHOT_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), TYPE_SLINGSHOT_WINDOW))
#define SLINGSHOT_WINDOW_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), TYPE_SLINGSHOT_WINDOW, SlingshotWindowClass))

typedef struct _SlingshotWindow SlingshotWindow;
typedef struct _SlingshotWindowClass SlingshotWindowClass;
typedef struct _SlingshotWindowPrivate SlingshotWindowPrivate;

#define SLINGSHOT_FRONTEND_TYPE_APP_ITEM (slingshot_frontend_app_item_get_type ())
#define SLINGSHOT_FRONTEND_APP_ITEM(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), SLINGSHOT_FRONTEND_TYPE_APP_ITEM, SlingshotFrontendAppItem))
#define SLINGSHOT_FRONTEND_APP_ITEM_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), SLINGSHOT_FRONTEND_TYPE_APP_ITEM, SlingshotFrontendAppItemClass))
#define SLINGSHOT_FRONTEND_IS_APP_ITEM(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), SLINGSHOT_FRONTEND_TYPE_APP_ITEM))
#define SLINGSHOT_FRONTEND_IS_APP_ITEM_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), SLINGSHOT_FRONTEND_TYPE_APP_ITEM))
#define SLINGSHOT_FRONTEND_APP_ITEM_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), SLINGSHOT_FRONTEND_TYPE_APP_ITEM, SlingshotFrontendAppItemClass))

typedef struct _SlingshotFrontendAppItem SlingshotFrontendAppItem;
typedef struct _SlingshotFrontendAppItemClass SlingshotFrontendAppItemClass;

#define SLINGSHOT_FRONTEND_TYPE_SEARCHBAR (slingshot_frontend_searchbar_get_type ())
#define SLINGSHOT_FRONTEND_SEARCHBAR(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), SLINGSHOT_FRONTEND_TYPE_SEARCHBAR, SlingshotFrontendSearchbar))
#define SLINGSHOT_FRONTEND_SEARCHBAR_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), SLINGSHOT_FRONTEND_TYPE_SEARCHBAR, SlingshotFrontendSearchbarClass))
#define SLINGSHOT_FRONTEND_IS_SEARCHBAR(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), SLINGSHOT_FRONTEND_TYPE_SEARCHBAR))
#define SLINGSHOT_FRONTEND_IS_SEARCHBAR_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), SLINGSHOT_FRONTEND_TYPE_SEARCHBAR))
#define SLINGSHOT_FRONTEND_SEARCHBAR_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), SLINGSHOT_FRONTEND_TYPE_SEARCHBAR, SlingshotFrontendSearchbarClass))

typedef struct _SlingshotFrontendSearchbar SlingshotFrontendSearchbar;
typedef struct _SlingshotFrontendSearchbarClass SlingshotFrontendSearchbarClass;

#define SLINGSHOT_FRONTEND_TYPE_GRID (slingshot_frontend_grid_get_type ())
#define SLINGSHOT_FRONTEND_GRID(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), SLINGSHOT_FRONTEND_TYPE_GRID, SlingshotFrontendGrid))
#define SLINGSHOT_FRONTEND_GRID_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), SLINGSHOT_FRONTEND_TYPE_GRID, SlingshotFrontendGridClass))
#define SLINGSHOT_FRONTEND_IS_GRID(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), SLINGSHOT_FRONTEND_TYPE_GRID))
#define SLINGSHOT_FRONTEND_IS_GRID_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), SLINGSHOT_FRONTEND_TYPE_GRID))
#define SLINGSHOT_FRONTEND_GRID_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), SLINGSHOT_FRONTEND_TYPE_GRID, SlingshotFrontendGridClass))

typedef struct _SlingshotFrontendGrid SlingshotFrontendGrid;
typedef struct _SlingshotFrontendGridClass SlingshotFrontendGridClass;

#define SLINGSHOT_FRONTEND_TYPE_INDICATORS (slingshot_frontend_indicators_get_type ())
#define SLINGSHOT_FRONTEND_INDICATORS(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), SLINGSHOT_FRONTEND_TYPE_INDICATORS, SlingshotFrontendIndicators))
#define SLINGSHOT_FRONTEND_INDICATORS_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST ((klass), SLINGSHOT_FRONTEND_TYPE_INDICATORS, SlingshotFrontendIndicatorsClass))
#define SLINGSHOT_FRONTEND_IS_INDICATORS(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), SLINGSHOT_FRONTEND_TYPE_INDICATORS))
#define SLINGSHOT_FRONTEND_IS_INDICATORS_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((klass), SLINGSHOT_FRONTEND_TYPE_INDICATORS))
#define SLINGSHOT_FRONTEND_INDICATORS_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS ((obj), SLINGSHOT_FRONTEND_TYPE_INDICATORS, SlingshotFrontendIndicatorsClass))

typedef struct _SlingshotFrontendIndicators SlingshotFrontendIndicators;
typedef struct _SlingshotFrontendIndicatorsClass SlingshotFrontendIndicatorsClass;
#define __g_list_free__g_object_unref0_0(var) ((var == NULL) ? NULL : (var = (_g_list_free__g_object_unref0_ (var), NULL)))
#define _g_object_unref0(var) ((var == NULL) ? NULL : (var = (g_object_unref (var), NULL)))
#define _g_free0(var) (var = (g_free (var), NULL))
typedef struct _Block1Data Block1Data;
typedef struct _SlingshotFrontendIndicatorsPrivate SlingshotFrontendIndicatorsPrivate;
#define _g_error_free0(var) ((var == NULL) ? NULL : (var = (g_error_free (var), NULL)))
#define _gmenu_tree_item_unref0(var) ((var == NULL) ? NULL : (var = (gmenu_tree_item_unref (var), NULL)))
#define _cairo_pattern_destroy0(var) ((var == NULL) ? NULL : (var = (cairo_pattern_destroy (var), NULL)))
#define _cairo_destroy0(var) ((var == NULL) ? NULL : (var = (cairo_destroy (var), NULL)))
#define _g_list_free0(var) ((var == NULL) ? NULL : (var = (g_list_free (var), NULL)))
#define _unique_message_data_free0(var) ((var == NULL) ? NULL : (var = (unique_message_data_free (var), NULL)))

struct _ElementaryWidgetsCompositedWindow {
	GtkWindow parent_instance;
	ElementaryWidgetsCompositedWindowPrivate * priv;
};

struct _ElementaryWidgetsCompositedWindowClass {
	GtkWindowClass parent_class;
};

struct _SlingshotWindow {
	ElementaryWidgetsCompositedWindow parent_instance;
	SlingshotWindowPrivate * priv;
	GList* children;
	SlingshotFrontendSearchbar* searchbar;
	SlingshotFrontendGrid* grid;
	GeeArrayList* apps;
	GeeHashMap* icons;
	GeeArrayList* filtered;
	SlingshotFrontendIndicators* pages;
	SlingshotFrontendIndicators* categories;
	GeeArrayList* all_categories;
	gint icon_size;
	gint total_pages;
	GtkHBox* top_spacer;
};

struct _SlingshotWindowClass {
	ElementaryWidgetsCompositedWindowClass parent_class;
};

struct _Block1Data {
	int _ref_count_;
	SlingshotWindow* self;
	SlingshotFrontendAppItem* item;
};

struct _SlingshotFrontendIndicators {
	GtkHBox parent_instance;
	SlingshotFrontendIndicatorsPrivate * priv;
	GList* children;
	gint active;
};

struct _SlingshotFrontendIndicatorsClass {
	GtkHBoxClass parent_class;
};


static gpointer slingshot_window_parent_class = NULL;

GType elementary_widgets_composited_window_get_type (void) G_GNUC_CONST;
GType slingshot_window_get_type (void) G_GNUC_CONST;
GType slingshot_frontend_app_item_get_type (void) G_GNUC_CONST;
GType slingshot_frontend_searchbar_get_type (void) G_GNUC_CONST;
GType slingshot_frontend_grid_get_type (void) G_GNUC_CONST;
GType slingshot_frontend_indicators_get_type (void) G_GNUC_CONST;
enum  {
	SLINGSHOT_WINDOW_DUMMY_PROPERTY
};
static void _g_object_unref0_ (gpointer var);
static void _g_list_free__g_object_unref0_ (GList* self);
GeeArrayList* slingshot_backend_gmenu_entries_get_categories (void);
SlingshotWindow* slingshot_window_new (void);
SlingshotWindow* slingshot_window_construct (GType object_type);
ElementaryWidgetsCompositedWindow* elementary_widgets_composited_window_new (void);
ElementaryWidgetsCompositedWindow* elementary_widgets_composited_window_construct (GType object_type);
void slingshot_backend_gmenu_entries_enumerate_apps (GeeArrayList* source, GeeHashMap* icons, gint icon_size, GeeArrayList** list);
GeeArrayList* slingshot_backend_gmenu_entries_get_all (void);
static void __lambda13_ (SlingshotWindow* self);
static void ___lambda13__gtk_widget_realize (GtkWidget* _sender, gpointer self);
SlingshotFrontendSearchbar* slingshot_frontend_searchbar_new (const gchar* hint);
SlingshotFrontendSearchbar* slingshot_frontend_searchbar_construct (GType object_type, const gchar* hint);
static void slingshot_window_search (SlingshotWindow* self);
static void _slingshot_window_search_slingshot_frontend_searchbar_changed (SlingshotFrontendSearchbar* _sender, gpointer self);
SlingshotFrontendGrid* slingshot_frontend_grid_new (gint rows, gint columns);
SlingshotFrontendGrid* slingshot_frontend_grid_construct (GType object_type, gint rows, gint columns);
static void slingshot_window_populate_grid (SlingshotWindow* self);
SlingshotFrontendIndicators* slingshot_frontend_indicators_new (void);
SlingshotFrontendIndicators* slingshot_frontend_indicators_construct (GType object_type);
static void __lambda18_ (SlingshotWindow* self);
static void slingshot_window_update_grid (SlingshotWindow* self, GeeArrayList* apps);
static void ___lambda18__slingshot_frontend_indicators_child_activated (SlingshotFrontendIndicators* _sender, gpointer self);
static void slingshot_window_update_pages (SlingshotWindow* self, GeeArrayList* apps);
void slingshot_frontend_indicators_append (SlingshotFrontendIndicators* self, const gchar* thelabel);
void slingshot_frontend_indicators_set_active (SlingshotFrontendIndicators* self, gint index);
static gboolean __lambda19_ (SlingshotWindow* self);
void slingshot_window_destroy (SlingshotWindow* self);
static gboolean ___lambda19__gtk_widget_button_release_event (GtkWidget* _sender, GdkEventButton* event, gpointer self);
static gboolean slingshot_window_draw_background (SlingshotWindow* self, GtkWidget* widget, GdkEventExpose* event);
static gboolean _slingshot_window_draw_background_gtk_widget_expose_event (GtkWidget* _sender, GdkEventExpose* event, gpointer self);
static gboolean __lambda20_ (SlingshotWindow* self);
static gboolean ___lambda20__gtk_widget_focus_out_event (GtkWidget* _sender, GdkEventFocus* event, gpointer self);
static Block1Data* block1_data_ref (Block1Data* _data1_);
static void block1_data_unref (void * _userdata_);
SlingshotFrontendAppItem* slingshot_frontend_app_item_new (gint size);
SlingshotFrontendAppItem* slingshot_frontend_app_item_construct (GType object_type, gint size);
static gboolean ________lambda14_ (Block1Data* _data1_);
static gboolean _________lambda14__gtk_widget_button_press_event (GtkWidget* _sender, GdkEventButton* event, gpointer self);
static gboolean ________lambda15_ (Block1Data* _data1_);
static gboolean _________lambda15__gtk_widget_enter_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self);
static gboolean ________lambda16_ (SlingshotWindow* self);
static gboolean _________lambda16__gtk_widget_leave_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self);
static gboolean ________lambda17_ (Block1Data* _data1_);
static gboolean _________lambda17__gtk_widget_button_release_event (GtkWidget* _sender, GdkEventButton* event, gpointer self);
void slingshot_frontend_app_item_change_app (SlingshotFrontendAppItem* self, GdkPixbuf* new_icon, const gchar* new_name, const gchar* new_tooltip);
static void slingshot_window_change_category (SlingshotWindow* self);
GeeArrayList* slingshot_backend_gmenu_entries_get_applications_for_category (GMenuTreeDirectory* category);
gchar* slingshot_frontend_searchbar_get_text (SlingshotFrontendSearchbar* self);
void slingshot_frontend_indicators_set_active_no_signal (SlingshotFrontendIndicators* self, gint index);
static void slingshot_window_page_left (SlingshotWindow* self);
static void slingshot_window_page_right (SlingshotWindow* self);
static gboolean slingshot_window_real_key_press_event (GtkWidget* base, GdkEventKey* event);
void slingshot_frontend_searchbar_set_text (SlingshotFrontendSearchbar* self, const gchar* value);
static gboolean slingshot_window_real_scroll_event (GtkWidget* base, GdkEventScroll* event);
const gchar* gdk_scroll_direction_to_string (GdkScrollDirection self);
static void slingshot_window_finalize (GObject* obj);
gint _vala_main (gchar** args, int args_length1);


static void _g_object_unref0_ (gpointer var) {
	(var == NULL) ? NULL : (var = (g_object_unref (var), NULL));
}


static void _g_list_free__g_object_unref0_ (GList* self) {
	g_list_foreach (self, (GFunc) _g_object_unref0_, NULL);
	g_list_free (self);
}


static gpointer _g_object_ref0 (gpointer self) {
	return self ? g_object_ref (self) : NULL;
}


static void __lambda13_ (SlingshotWindow* self) {
	GtkHBox* _tmp0_ = NULL;
	_tmp0_ = self->top_spacer;
	gtk_widget_set_visible ((GtkWidget*) _tmp0_, TRUE);
}


static void ___lambda13__gtk_widget_realize (GtkWidget* _sender, gpointer self) {
	__lambda13_ ((SlingshotWindow*) self);
}


static void _slingshot_window_search_slingshot_frontend_searchbar_changed (SlingshotFrontendSearchbar* _sender, gpointer self) {
	slingshot_window_search ((SlingshotWindow*) self);
}


static void __lambda18_ (SlingshotWindow* self) {
	GeeArrayList* _tmp0_ = NULL;
	_tmp0_ = self->filtered;
	slingshot_window_update_grid (self, _tmp0_);
}


static void ___lambda18__slingshot_frontend_indicators_child_activated (SlingshotFrontendIndicators* _sender, gpointer self) {
	__lambda18_ ((SlingshotWindow*) self);
}


static gboolean __lambda19_ (SlingshotWindow* self) {
	gboolean result = FALSE;
	slingshot_window_destroy (self);
	result = FALSE;
	return result;
}


static gboolean ___lambda19__gtk_widget_button_release_event (GtkWidget* _sender, GdkEventButton* event, gpointer self) {
	gboolean result;
	result = __lambda19_ ((SlingshotWindow*) self);
	return result;
}


static gboolean _slingshot_window_draw_background_gtk_widget_expose_event (GtkWidget* _sender, GdkEventExpose* event, gpointer self) {
	gboolean result;
	result = slingshot_window_draw_background ((SlingshotWindow*) self, _sender, event);
	return result;
}


static gboolean __lambda20_ (SlingshotWindow* self) {
	gboolean result = FALSE;
	slingshot_window_destroy (self);
	result = TRUE;
	return result;
}


static gboolean ___lambda20__gtk_widget_focus_out_event (GtkWidget* _sender, GdkEventFocus* event, gpointer self) {
	gboolean result;
	result = __lambda20_ ((SlingshotWindow*) self);
	return result;
}


SlingshotWindow* slingshot_window_construct (GType object_type) {
	SlingshotWindow * self = NULL;
	WnckScreen* _tmp0_ = NULL;
	GdkRectangle monitor_dimensions = {0};
	GdkScreen* screen = NULL;
	GdkScreen* _tmp1_ = NULL;
	GdkScreen* _tmp2_ = NULL;
	GdkScreen* _tmp3_ = NULL;
	GdkScreen* _tmp4_ = NULL;
	gint _tmp5_ = 0;
	GdkRectangle _tmp6_ = {0};
	gdouble suggested_size = 0.0;
	GdkRectangle _tmp7_ = {0};
	gint _tmp8_ = 0;
	GdkRectangle _tmp9_ = {0};
	gint _tmp10_ = 0;
	gdouble _tmp11_ = 0.0;
	gdouble _tmp12_ = 0.0;
	GeeArrayList* _tmp20_ = NULL;
	GeeArrayList* _tmp21_ = NULL;
	GeeHashMap* _tmp22_ = NULL;
	gint _tmp23_ = 0;
	GeeArrayList* _tmp24_ = NULL;
	GtkEventBox* wrapper = NULL;
	GtkEventBox* _tmp25_ = NULL;
	GtkEventBox* _tmp26_ = NULL;
	GtkEventBox* _tmp27_ = NULL;
	GtkVBox* container = NULL;
	GtkVBox* _tmp28_ = NULL;
	GtkEventBox* _tmp29_ = NULL;
	GtkVBox* _tmp30_ = NULL;
	GtkHBox* top = NULL;
	GtkHBox* _tmp31_ = NULL;
	GtkHBox* bottom = NULL;
	GtkHBox* _tmp32_ = NULL;
	GtkHBox* _tmp33_ = NULL;
	GtkHBox* _tmp34_ = NULL;
	GtkHBox* _tmp35_ = NULL;
	GtkHBox* _tmp36_ = NULL;
	GtkHBox* _tmp37_ = NULL;
	SlingshotFrontendSearchbar* _tmp38_ = NULL;
	SlingshotFrontendSearchbar* _tmp39_ = NULL;
	gint medio = 0;
	GdkRectangle _tmp40_ = {0};
	gint _tmp41_ = 0;
	GtkHBox* _tmp42_ = NULL;
	SlingshotFrontendSearchbar* _tmp43_ = NULL;
	gint _tmp44_ = 0;
	GtkVBox* _tmp45_ = NULL;
	GtkHBox* _tmp46_ = NULL;
	GtkVBox* _tmp47_ = NULL;
	GtkHBox* _tmp48_ = NULL;
	GdkRectangle _tmp49_ = {0};
	gint _tmp50_ = 0;
	GdkRectangle _tmp51_ = {0};
	gint _tmp52_ = 0;
	GtkVBox* _tmp55_ = NULL;
	SlingshotFrontendGrid* _tmp56_ = NULL;
	SlingshotFrontendIndicators* _tmp57_ = NULL;
	SlingshotFrontendIndicators* _tmp58_ = NULL;
	GtkHBox* pages_wrapper = NULL;
	GtkHBox* _tmp59_ = NULL;
	GtkHBox* _tmp60_ = NULL;
	GtkVBox* _tmp61_ = NULL;
	GtkHBox* _tmp62_ = NULL;
	GeeArrayList* _tmp63_ = NULL;
	gint _tmp64_ = 0;
	SlingshotFrontendIndicators* _tmp75_ = NULL;
	self = (SlingshotWindow*) elementary_widgets_composited_window_construct (object_type);
	_tmp0_ = wnck_screen_get_default ();
	wnck_screen_toggle_showing_desktop (_tmp0_, FALSE);
	gtk_window_set_title ((GtkWindow*) self, "Slingshot");
	gtk_window_set_skip_pager_hint ((GtkWindow*) self, TRUE);
	gtk_window_set_skip_taskbar_hint ((GtkWindow*) self, TRUE);
	gtk_window_set_type_hint ((GtkWindow*) self, GDK_WINDOW_TYPE_HINT_NORMAL);
	gtk_window_maximize ((GtkWindow*) self);
	gtk_window_stick ((GtkWindow*) self);
	gtk_window_set_keep_above ((GtkWindow*) self, TRUE);
	_tmp1_ = gdk_screen_get_default ();
	_tmp2_ = _g_object_ref0 (_tmp1_);
	screen = _tmp2_;
	_tmp3_ = screen;
	_tmp4_ = screen;
	_tmp5_ = gdk_screen_get_primary_monitor (_tmp4_);
	gdk_screen_get_monitor_geometry (_tmp3_, _tmp5_, &_tmp6_);
	monitor_dimensions = _tmp6_;
	_tmp7_ = monitor_dimensions;
	_tmp8_ = _tmp7_.width;
	_tmp9_ = monitor_dimensions;
	_tmp10_ = _tmp9_.height;
	_tmp11_ = pow ((gdouble) (_tmp8_ * _tmp10_), (gdouble) (1.0 / 3.0));
	suggested_size = _tmp11_ / 1.6;
	_tmp12_ = suggested_size;
	if (_tmp12_ < ((gdouble) 27)) {
		self->icon_size = 24;
	} else {
		gboolean _tmp13_ = FALSE;
		gdouble _tmp14_ = 0.0;
		_tmp14_ = suggested_size;
		if (_tmp14_ >= ((gdouble) 27)) {
			gdouble _tmp15_ = 0.0;
			_tmp15_ = suggested_size;
			_tmp13_ = _tmp15_ < ((gdouble) 40);
		} else {
			_tmp13_ = FALSE;
		}
		if (_tmp13_) {
			self->icon_size = 32;
		} else {
			gboolean _tmp16_ = FALSE;
			gdouble _tmp17_ = 0.0;
			_tmp17_ = suggested_size;
			if (_tmp17_ >= ((gdouble) 40)) {
				gdouble _tmp18_ = 0.0;
				_tmp18_ = suggested_size;
				_tmp16_ = _tmp18_ < ((gdouble) 56);
			} else {
				_tmp16_ = FALSE;
			}
			if (_tmp16_) {
				self->icon_size = 48;
			} else {
				gdouble _tmp19_ = 0.0;
				_tmp19_ = suggested_size;
				if (_tmp19_ >= ((gdouble) 56)) {
					self->icon_size = 64;
				}
			}
		}
	}
	_tmp20_ = slingshot_backend_gmenu_entries_get_all ();
	_tmp21_ = _tmp20_;
	_tmp22_ = self->icons;
	_tmp23_ = self->icon_size;
	slingshot_backend_gmenu_entries_enumerate_apps (_tmp21_, _tmp22_, _tmp23_, &_tmp24_);
	_g_object_unref0 (self->apps);
	self->apps = _tmp24_;
	_g_object_unref0 (_tmp21_);
	_tmp25_ = (GtkEventBox*) gtk_event_box_new ();
	g_object_ref_sink (_tmp25_);
	wrapper = _tmp25_;
	_tmp26_ = wrapper;
	gtk_event_box_set_visible_window (_tmp26_, FALSE);
	_tmp27_ = wrapper;
	gtk_container_add ((GtkContainer*) self, (GtkWidget*) _tmp27_);
	_tmp28_ = (GtkVBox*) gtk_vbox_new (FALSE, 5);
	g_object_ref_sink (_tmp28_);
	container = _tmp28_;
	_tmp29_ = wrapper;
	_tmp30_ = container;
	gtk_container_add ((GtkContainer*) _tmp29_, (GtkWidget*) _tmp30_);
	_tmp31_ = (GtkHBox*) gtk_hbox_new (FALSE, 0);
	g_object_ref_sink (_tmp31_);
	top = _tmp31_;
	_tmp32_ = (GtkHBox*) gtk_hbox_new (FALSE, 0);
	g_object_ref_sink (_tmp32_);
	bottom = _tmp32_;
	_tmp33_ = (GtkHBox*) gtk_hbox_new (FALSE, 20);
	g_object_ref_sink (_tmp33_);
	_g_object_unref0 (self->top_spacer);
	self->top_spacer = _tmp33_;
	_tmp34_ = self->top_spacer;
	g_signal_connect_object ((GtkWidget*) _tmp34_, "realize", (GCallback) ___lambda13__gtk_widget_realize, self, 0);
	_tmp35_ = self->top_spacer;
	g_object_set ((GtkWidget*) _tmp35_, "can-focus", TRUE, NULL);
	_tmp36_ = bottom;
	_tmp37_ = self->top_spacer;
	gtk_box_pack_start ((GtkBox*) _tmp36_, (GtkWidget*) _tmp37_, FALSE, FALSE, (guint) 0);
	_tmp38_ = slingshot_frontend_searchbar_new ("Procure por ...");
	g_object_ref_sink (_tmp38_);
	_g_object_unref0 (self->searchbar);
	self->searchbar = _tmp38_;
	_tmp39_ = self->searchbar;
	g_signal_connect_object (_tmp39_, "changed", (GCallback) _slingshot_window_search_slingshot_frontend_searchbar_changed, self, 0);
	_tmp40_ = monitor_dimensions;
	_tmp41_ = _tmp40_.width;
	medio = (_tmp41_ / 2) - 120;
	_tmp42_ = bottom;
	_tmp43_ = self->searchbar;
	_tmp44_ = medio;
	gtk_box_pack_start ((GtkBox*) _tmp42_, (GtkWidget*) _tmp43_, FALSE, TRUE, (guint) _tmp44_);
	_tmp45_ = container;
	_tmp46_ = bottom;
	gtk_box_pack_start ((GtkBox*) _tmp45_, (GtkWidget*) _tmp46_, FALSE, TRUE, (guint) 20);
	_tmp47_ = container;
	_tmp48_ = top;
	gtk_box_pack_start ((GtkBox*) _tmp47_, (GtkWidget*) _tmp48_, FALSE, TRUE, (guint) 0);
	_tmp49_ = monitor_dimensions;
	_tmp50_ = _tmp49_.width;
	_tmp51_ = monitor_dimensions;
	_tmp52_ = _tmp51_.height;
	if (_tmp50_ > _tmp52_) {
		SlingshotFrontendGrid* _tmp53_ = NULL;
		_tmp53_ = slingshot_frontend_grid_new (4, 6);
		g_object_ref_sink (_tmp53_);
		_g_object_unref0 (self->grid);
		self->grid = _tmp53_;
	} else {
		SlingshotFrontendGrid* _tmp54_ = NULL;
		_tmp54_ = slingshot_frontend_grid_new (6, 4);
		g_object_ref_sink (_tmp54_);
		_g_object_unref0 (self->grid);
		self->grid = _tmp54_;
	}
	_tmp55_ = container;
	_tmp56_ = self->grid;
	gtk_box_pack_start ((GtkBox*) _tmp55_, (GtkWidget*) _tmp56_, TRUE, TRUE, (guint) 0);
	slingshot_window_populate_grid (self);
	_tmp57_ = slingshot_frontend_indicators_new ();
	g_object_ref_sink (_tmp57_);
	_g_object_unref0 (self->pages);
	self->pages = _tmp57_;
	_tmp58_ = self->pages;
	g_signal_connect_object (_tmp58_, "child-activated", (GCallback) ___lambda18__slingshot_frontend_indicators_child_activated, self, 0);
	_tmp59_ = (GtkHBox*) gtk_hbox_new (FALSE, 0);
	g_object_ref_sink (_tmp59_);
	pages_wrapper = _tmp59_;
	_tmp60_ = pages_wrapper;
	gtk_widget_set_size_request ((GtkWidget*) _tmp60_, -1, 30);
	_tmp61_ = container;
	_tmp62_ = pages_wrapper;
	gtk_box_pack_end ((GtkBox*) _tmp61_, (GtkWidget*) _tmp62_, FALSE, TRUE, (guint) 15);
	_tmp63_ = self->apps;
	slingshot_window_update_pages (self, _tmp63_);
	_tmp64_ = self->total_pages;
	if (_tmp64_ > 1) {
		GtkHBox* _tmp65_ = NULL;
		SlingshotFrontendIndicators* _tmp66_ = NULL;
		_tmp65_ = pages_wrapper;
		_tmp66_ = self->pages;
		gtk_box_pack_start ((GtkBox*) _tmp65_, (GtkWidget*) _tmp66_, TRUE, FALSE, (guint) 0);
		{
			gint p = 0;
			p = 1;
			{
				gboolean _tmp67_ = FALSE;
				_tmp67_ = TRUE;
				while (TRUE) {
					gint _tmp69_ = 0;
					gint _tmp70_ = 0;
					SlingshotFrontendIndicators* _tmp71_ = NULL;
					gint _tmp72_ = 0;
					gchar* _tmp73_ = NULL;
					gchar* _tmp74_ = NULL;
					if (!_tmp67_) {
						gint _tmp68_ = 0;
						_tmp68_ = p;
						p = _tmp68_ + 1;
					}
					_tmp67_ = FALSE;
					_tmp69_ = p;
					_tmp70_ = self->total_pages;
					if (!(_tmp69_ <= _tmp70_)) {
						break;
					}
					_tmp71_ = self->pages;
					_tmp72_ = p;
					_tmp73_ = g_strdup_printf ("%i", _tmp72_);
					_tmp74_ = _tmp73_;
					slingshot_frontend_indicators_append (_tmp71_, _tmp74_);
					_g_free0 (_tmp74_);
				}
			}
		}
	}
	_tmp75_ = self->pages;
	slingshot_frontend_indicators_set_active (_tmp75_, 0);
	g_signal_connect_object ((GtkWidget*) self, "button-release-event", (GCallback) ___lambda19__gtk_widget_button_release_event, self, 0);
	g_signal_connect_object ((GtkWidget*) self, "expose-event", (GCallback) _slingshot_window_draw_background_gtk_widget_expose_event, self, 0);
	g_signal_connect_object ((GtkWidget*) self, "focus-out-event", (GCallback) ___lambda20__gtk_widget_focus_out_event, self, 0);
	_g_object_unref0 (pages_wrapper);
	_g_object_unref0 (bottom);
	_g_object_unref0 (top);
	_g_object_unref0 (container);
	_g_object_unref0 (wrapper);
	_g_object_unref0 (screen);
	return self;
}


SlingshotWindow* slingshot_window_new (void) {
	return slingshot_window_construct (TYPE_SLINGSHOT_WINDOW);
}


static Block1Data* block1_data_ref (Block1Data* _data1_) {
	g_atomic_int_inc (&_data1_->_ref_count_);
	return _data1_;
}


static void block1_data_unref (void * _userdata_) {
	Block1Data* _data1_;
	_data1_ = (Block1Data*) _userdata_;
	if (g_atomic_int_dec_and_test (&_data1_->_ref_count_)) {
		SlingshotWindow* self;
		self = _data1_->self;
		_g_object_unref0 (_data1_->item);
		_g_object_unref0 (self);
		g_slice_free (Block1Data, _data1_);
	}
}


static gboolean ________lambda14_ (Block1Data* _data1_) {
	SlingshotWindow* self;
	gboolean result = FALSE;
	SlingshotFrontendAppItem* _tmp0_ = NULL;
	self = _data1_->self;
	_tmp0_ = _data1_->item;
	gtk_widget_grab_focus ((GtkWidget*) _tmp0_);
	result = TRUE;
	return result;
}


static gboolean _________lambda14__gtk_widget_button_press_event (GtkWidget* _sender, GdkEventButton* event, gpointer self) {
	gboolean result;
	result = ________lambda14_ (self);
	return result;
}


static gboolean ________lambda15_ (Block1Data* _data1_) {
	SlingshotWindow* self;
	gboolean result = FALSE;
	SlingshotFrontendAppItem* _tmp0_ = NULL;
	self = _data1_->self;
	_tmp0_ = _data1_->item;
	gtk_widget_grab_focus ((GtkWidget*) _tmp0_);
	result = TRUE;
	return result;
}


static gboolean _________lambda15__gtk_widget_enter_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self) {
	gboolean result;
	result = ________lambda15_ (self);
	return result;
}


static gboolean ________lambda16_ (SlingshotWindow* self) {
	gboolean result = FALSE;
	GtkHBox* _tmp0_ = NULL;
	_tmp0_ = self->top_spacer;
	gtk_widget_grab_focus ((GtkWidget*) _tmp0_);
	result = TRUE;
	return result;
}


static gboolean _________lambda16__gtk_widget_leave_notify_event (GtkWidget* _sender, GdkEventCrossing* event, gpointer self) {
	gboolean result;
	result = ________lambda16_ ((SlingshotWindow*) self);
	return result;
}


static gboolean ________lambda17_ (Block1Data* _data1_) {
	SlingshotWindow* self;
	gboolean result = FALSE;
	GError * _inner_error_ = NULL;
	self = _data1_->self;
	{
		GeeArrayList* _tmp0_ = NULL;
		GList* _tmp1_ = NULL;
		SlingshotFrontendAppItem* _tmp2_ = NULL;
		gint _tmp3_ = 0;
		SlingshotFrontendIndicators* _tmp4_ = NULL;
		gint _tmp5_ = 0;
		SlingshotFrontendGrid* _tmp6_ = NULL;
		guint _tmp7_ = 0U;
		guint _tmp8_ = 0U;
		SlingshotFrontendGrid* _tmp9_ = NULL;
		guint _tmp10_ = 0U;
		guint _tmp11_ = 0U;
		gpointer _tmp12_ = NULL;
		GeeHashMap* _tmp13_ = NULL;
		gpointer _tmp14_ = NULL;
		gchar* _tmp15_ = NULL;
		GDesktopAppInfo* _tmp16_ = NULL;
		GDesktopAppInfo* _tmp17_ = NULL;
		_tmp0_ = self->filtered;
		_tmp1_ = self->children;
		_tmp2_ = _data1_->item;
		_tmp3_ = g_list_index (_tmp1_, _tmp2_);
		_tmp4_ = self->pages;
		_tmp5_ = _tmp4_->active;
		_tmp6_ = self->grid;
		g_object_get ((GtkTable*) _tmp6_, "n-columns", &_tmp7_, NULL);
		_tmp8_ = _tmp7_;
		_tmp9_ = self->grid;
		g_object_get ((GtkTable*) _tmp9_, "n-rows", &_tmp10_, NULL);
		_tmp11_ = _tmp10_;
		_tmp12_ = gee_abstract_list_get ((GeeAbstractList*) _tmp0_, (gint) (_tmp3_ + ((_tmp5_ * _tmp8_) * _tmp11_)));
		_tmp13_ = (GeeHashMap*) _tmp12_;
		_tmp14_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp13_, "desktop_file");
		_tmp15_ = (gchar*) _tmp14_;
		_tmp16_ = g_desktop_app_info_new_from_filename (_tmp15_);
		_tmp17_ = _tmp16_;
		g_app_info_launch ((GAppInfo*) _tmp17_, NULL, NULL, &_inner_error_);
		_g_object_unref0 (_tmp17_);
		_g_free0 (_tmp15_);
		_g_object_unref0 (_tmp13_);
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			goto __catch0_g_error;
		}
		slingshot_window_destroy (self);
	}
	goto __finally0;
	__catch0_g_error:
	{
		GError* e = NULL;
		FILE* _tmp18_ = NULL;
		GError* _tmp19_ = NULL;
		const gchar* _tmp20_ = NULL;
		gchar* _tmp21_ = NULL;
		gchar* _tmp22_ = NULL;
		e = _inner_error_;
		_inner_error_ = NULL;
		_tmp18_ = stdout;
		_tmp19_ = e;
		_tmp20_ = _tmp19_->message;
		_tmp21_ = g_strconcat ("Error! Load application: ", _tmp20_, NULL);
		_tmp22_ = _tmp21_;
		fprintf (_tmp18_, "%s", _tmp22_);
		_g_free0 (_tmp22_);
		_g_error_free0 (e);
	}
	__finally0:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_critical ("file %s: line %d: uncaught error: %s (%s, %d)", __FILE__, __LINE__, _inner_error_->message, g_quark_to_string (_inner_error_->domain), _inner_error_->code);
		g_clear_error (&_inner_error_);
		return FALSE;
	}
	result = TRUE;
	return result;
}


static gboolean _________lambda17__gtk_widget_button_release_event (GtkWidget* _sender, GdkEventButton* event, gpointer self) {
	gboolean result;
	result = ________lambda17_ (self);
	return result;
}


static void slingshot_window_populate_grid (SlingshotWindow* self) {
	g_return_if_fail (self != NULL);
	{
		gint r = 0;
		r = 0;
		{
			gboolean _tmp0_ = FALSE;
			_tmp0_ = TRUE;
			while (TRUE) {
				gint _tmp2_ = 0;
				SlingshotFrontendGrid* _tmp3_ = NULL;
				guint _tmp4_ = 0U;
				guint _tmp5_ = 0U;
				if (!_tmp0_) {
					gint _tmp1_ = 0;
					_tmp1_ = r;
					r = _tmp1_ + 1;
				}
				_tmp0_ = FALSE;
				_tmp2_ = r;
				_tmp3_ = self->grid;
				g_object_get ((GtkTable*) _tmp3_, "n-rows", &_tmp4_, NULL);
				_tmp5_ = _tmp4_;
				if (!(((guint) _tmp2_) < _tmp5_)) {
					break;
				}
				{
					gint c = 0;
					c = 0;
					{
						gboolean _tmp6_ = FALSE;
						_tmp6_ = TRUE;
						while (TRUE) {
							Block1Data* _data1_;
							gint _tmp8_ = 0;
							SlingshotFrontendGrid* _tmp9_ = NULL;
							guint _tmp10_ = 0U;
							guint _tmp11_ = 0U;
							gint _tmp12_ = 0;
							SlingshotFrontendAppItem* _tmp13_ = NULL;
							SlingshotFrontendAppItem* _tmp14_ = NULL;
							SlingshotFrontendAppItem* _tmp15_ = NULL;
							SlingshotFrontendAppItem* _tmp16_ = NULL;
							SlingshotFrontendAppItem* _tmp17_ = NULL;
							SlingshotFrontendAppItem* _tmp18_ = NULL;
							SlingshotFrontendAppItem* _tmp19_ = NULL;
							SlingshotFrontendGrid* _tmp20_ = NULL;
							SlingshotFrontendAppItem* _tmp21_ = NULL;
							gint _tmp22_ = 0;
							gint _tmp23_ = 0;
							gint _tmp24_ = 0;
							gint _tmp25_ = 0;
							_data1_ = g_slice_new0 (Block1Data);
							_data1_->_ref_count_ = 1;
							_data1_->self = g_object_ref (self);
							if (!_tmp6_) {
								gint _tmp7_ = 0;
								_tmp7_ = c;
								c = _tmp7_ + 1;
							}
							_tmp6_ = FALSE;
							_tmp8_ = c;
							_tmp9_ = self->grid;
							g_object_get ((GtkTable*) _tmp9_, "n-columns", &_tmp10_, NULL);
							_tmp11_ = _tmp10_;
							if (!(((guint) _tmp8_) < _tmp11_)) {
								block1_data_unref (_data1_);
								_data1_ = NULL;
								break;
							}
							_tmp12_ = self->icon_size;
							_tmp13_ = slingshot_frontend_app_item_new (_tmp12_);
							g_object_ref_sink (_tmp13_);
							_data1_->item = _tmp13_;
							_tmp14_ = _data1_->item;
							_tmp15_ = _g_object_ref0 (_tmp14_);
							self->children = g_list_append (self->children, _tmp15_);
							_tmp16_ = _data1_->item;
							g_signal_connect_data ((GtkWidget*) _tmp16_, "button-press-event", (GCallback) _________lambda14__gtk_widget_button_press_event, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
							_tmp17_ = _data1_->item;
							g_signal_connect_data ((GtkWidget*) _tmp17_, "enter-notify-event", (GCallback) _________lambda15__gtk_widget_enter_notify_event, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
							_tmp18_ = _data1_->item;
							g_signal_connect_object ((GtkWidget*) _tmp18_, "leave-notify-event", (GCallback) _________lambda16__gtk_widget_leave_notify_event, self, 0);
							_tmp19_ = _data1_->item;
							g_signal_connect_data ((GtkWidget*) _tmp19_, "button-release-event", (GCallback) _________lambda17__gtk_widget_button_release_event, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
							_tmp20_ = self->grid;
							_tmp21_ = _data1_->item;
							_tmp22_ = c;
							_tmp23_ = c;
							_tmp24_ = r;
							_tmp25_ = r;
							gtk_table_attach ((GtkTable*) _tmp20_, (GtkWidget*) _tmp21_, (guint) _tmp22_, (guint) (_tmp23_ + 1), (guint) _tmp24_, (guint) (_tmp25_ + 1), GTK_EXPAND, GTK_EXPAND, (guint) 0, (guint) 0);
							block1_data_unref (_data1_);
							_data1_ = NULL;
						}
					}
				}
			}
		}
	}
}


static void slingshot_window_update_grid (SlingshotWindow* self, GeeArrayList* apps) {
	gint item_iter = 0;
	SlingshotFrontendIndicators* _tmp0_ = NULL;
	gint _tmp1_ = 0;
	SlingshotFrontendGrid* _tmp2_ = NULL;
	guint _tmp3_ = 0U;
	guint _tmp4_ = 0U;
	SlingshotFrontendGrid* _tmp5_ = NULL;
	guint _tmp6_ = 0U;
	guint _tmp7_ = 0U;
	GeeArrayList* _tmp80_ = NULL;
	GList* _tmp81_ = NULL;
	gconstpointer _tmp82_ = NULL;
	g_return_if_fail (self != NULL);
	g_return_if_fail (apps != NULL);
	_tmp0_ = self->pages;
	_tmp1_ = _tmp0_->active;
	_tmp2_ = self->grid;
	g_object_get ((GtkTable*) _tmp2_, "n-columns", &_tmp3_, NULL);
	_tmp4_ = _tmp3_;
	_tmp5_ = self->grid;
	g_object_get ((GtkTable*) _tmp5_, "n-rows", &_tmp6_, NULL);
	_tmp7_ = _tmp6_;
	item_iter = (gint) ((_tmp1_ * _tmp4_) * _tmp7_);
	{
		gint r = 0;
		r = 0;
		{
			gboolean _tmp8_ = FALSE;
			_tmp8_ = TRUE;
			while (TRUE) {
				gint _tmp10_ = 0;
				SlingshotFrontendGrid* _tmp11_ = NULL;
				guint _tmp12_ = 0U;
				guint _tmp13_ = 0U;
				if (!_tmp8_) {
					gint _tmp9_ = 0;
					_tmp9_ = r;
					r = _tmp9_ + 1;
				}
				_tmp8_ = FALSE;
				_tmp10_ = r;
				_tmp11_ = self->grid;
				g_object_get ((GtkTable*) _tmp11_, "n-rows", &_tmp12_, NULL);
				_tmp13_ = _tmp12_;
				if (!(((guint) _tmp10_) < _tmp13_)) {
					break;
				}
				{
					gint c = 0;
					c = 0;
					{
						gboolean _tmp14_ = FALSE;
						_tmp14_ = TRUE;
						while (TRUE) {
							gint _tmp16_ = 0;
							SlingshotFrontendGrid* _tmp17_ = NULL;
							guint _tmp18_ = 0U;
							guint _tmp19_ = 0U;
							gint table_pos = 0;
							gint _tmp20_ = 0;
							gint _tmp21_ = 0;
							SlingshotFrontendGrid* _tmp22_ = NULL;
							guint _tmp23_ = 0U;
							guint _tmp24_ = 0U;
							SlingshotFrontendAppItem* item = NULL;
							GList* _tmp25_ = NULL;
							gint _tmp26_ = 0;
							gconstpointer _tmp27_ = NULL;
							SlingshotFrontendAppItem* _tmp28_ = NULL;
							gint _tmp29_ = 0;
							GeeArrayList* _tmp30_ = NULL;
							gint _tmp31_ = 0;
							gint _tmp32_ = 0;
							gint _tmp79_ = 0;
							if (!_tmp14_) {
								gint _tmp15_ = 0;
								_tmp15_ = c;
								c = _tmp15_ + 1;
							}
							_tmp14_ = FALSE;
							_tmp16_ = c;
							_tmp17_ = self->grid;
							g_object_get ((GtkTable*) _tmp17_, "n-columns", &_tmp18_, NULL);
							_tmp19_ = _tmp18_;
							if (!(((guint) _tmp16_) < _tmp19_)) {
								break;
							}
							_tmp20_ = c;
							_tmp21_ = r;
							_tmp22_ = self->grid;
							g_object_get ((GtkTable*) _tmp22_, "n-columns", &_tmp23_, NULL);
							_tmp24_ = _tmp23_;
							table_pos = _tmp20_ + (_tmp21_ * ((gint) _tmp24_));
							_tmp25_ = self->children;
							_tmp26_ = table_pos;
							_tmp27_ = g_list_nth_data (_tmp25_, (guint) _tmp26_);
							_tmp28_ = _g_object_ref0 ((SlingshotFrontendAppItem*) _tmp27_);
							item = _tmp28_;
							_tmp29_ = item_iter;
							_tmp30_ = apps;
							_tmp31_ = gee_abstract_collection_get_size ((GeeCollection*) _tmp30_);
							_tmp32_ = _tmp31_;
							if (_tmp29_ < _tmp32_) {
								GeeHashMap* current_item = NULL;
								GeeArrayList* _tmp33_ = NULL;
								gint _tmp34_ = 0;
								gpointer _tmp35_ = NULL;
								gboolean _tmp36_ = FALSE;
								GeeHashMap* _tmp37_ = NULL;
								gpointer _tmp38_ = NULL;
								gchar* _tmp39_ = NULL;
								gboolean _tmp40_ = FALSE;
								SlingshotFrontendAppItem* _tmp77_ = NULL;
								_tmp33_ = apps;
								_tmp34_ = item_iter;
								_tmp35_ = gee_abstract_list_get ((GeeAbstractList*) _tmp33_, _tmp34_);
								current_item = (GeeHashMap*) _tmp35_;
								_tmp37_ = current_item;
								_tmp38_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp37_, "description");
								_tmp39_ = (gchar*) _tmp38_;
								_tmp40_ = _tmp39_ == NULL;
								_g_free0 (_tmp39_);
								if (_tmp40_) {
									_tmp36_ = TRUE;
								} else {
									GeeHashMap* _tmp41_ = NULL;
									gpointer _tmp42_ = NULL;
									gchar* _tmp43_ = NULL;
									_tmp41_ = current_item;
									_tmp42_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp41_, "description");
									_tmp43_ = (gchar*) _tmp42_;
									_tmp36_ = g_strcmp0 (_tmp43_, "") == 0;
									_g_free0 (_tmp43_);
								}
								if (_tmp36_) {
									SlingshotFrontendAppItem* _tmp44_ = NULL;
									GeeHashMap* _tmp45_ = NULL;
									GeeHashMap* _tmp46_ = NULL;
									gpointer _tmp47_ = NULL;
									gchar* _tmp48_ = NULL;
									gpointer _tmp49_ = NULL;
									GdkPixbuf* _tmp50_ = NULL;
									GeeHashMap* _tmp51_ = NULL;
									gpointer _tmp52_ = NULL;
									gchar* _tmp53_ = NULL;
									GeeHashMap* _tmp54_ = NULL;
									gpointer _tmp55_ = NULL;
									gchar* _tmp56_ = NULL;
									_tmp44_ = item;
									_tmp45_ = self->icons;
									_tmp46_ = current_item;
									_tmp47_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp46_, "command");
									_tmp48_ = (gchar*) _tmp47_;
									_tmp49_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp45_, _tmp48_);
									_tmp50_ = (GdkPixbuf*) _tmp49_;
									_tmp51_ = current_item;
									_tmp52_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp51_, "name");
									_tmp53_ = (gchar*) _tmp52_;
									_tmp54_ = current_item;
									_tmp55_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp54_, "name");
									_tmp56_ = (gchar*) _tmp55_;
									slingshot_frontend_app_item_change_app (_tmp44_, _tmp50_, _tmp53_, _tmp56_);
									_g_free0 (_tmp56_);
									_g_free0 (_tmp53_);
									_g_object_unref0 (_tmp50_);
									_g_free0 (_tmp48_);
								} else {
									SlingshotFrontendAppItem* _tmp57_ = NULL;
									GeeHashMap* _tmp58_ = NULL;
									GeeHashMap* _tmp59_ = NULL;
									gpointer _tmp60_ = NULL;
									gchar* _tmp61_ = NULL;
									gpointer _tmp62_ = NULL;
									GdkPixbuf* _tmp63_ = NULL;
									GeeHashMap* _tmp64_ = NULL;
									gpointer _tmp65_ = NULL;
									gchar* _tmp66_ = NULL;
									GeeHashMap* _tmp67_ = NULL;
									gpointer _tmp68_ = NULL;
									gchar* _tmp69_ = NULL;
									gchar* _tmp70_ = NULL;
									gchar* _tmp71_ = NULL;
									GeeHashMap* _tmp72_ = NULL;
									gpointer _tmp73_ = NULL;
									gchar* _tmp74_ = NULL;
									gchar* _tmp75_ = NULL;
									gchar* _tmp76_ = NULL;
									_tmp57_ = item;
									_tmp58_ = self->icons;
									_tmp59_ = current_item;
									_tmp60_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp59_, "command");
									_tmp61_ = (gchar*) _tmp60_;
									_tmp62_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp58_, _tmp61_);
									_tmp63_ = (GdkPixbuf*) _tmp62_;
									_tmp64_ = current_item;
									_tmp65_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp64_, "name");
									_tmp66_ = (gchar*) _tmp65_;
									_tmp67_ = current_item;
									_tmp68_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp67_, "name");
									_tmp69_ = (gchar*) _tmp68_;
									_tmp70_ = g_strconcat (_tmp69_, ":\n", NULL);
									_tmp71_ = _tmp70_;
									_tmp72_ = current_item;
									_tmp73_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp72_, "description");
									_tmp74_ = (gchar*) _tmp73_;
									_tmp75_ = g_strconcat (_tmp71_, _tmp74_, NULL);
									_tmp76_ = _tmp75_;
									slingshot_frontend_app_item_change_app (_tmp57_, _tmp63_, _tmp66_, _tmp76_);
									_g_free0 (_tmp76_);
									_g_free0 (_tmp74_);
									_g_free0 (_tmp71_);
									_g_free0 (_tmp69_);
									_g_free0 (_tmp66_);
									_g_object_unref0 (_tmp63_);
									_g_free0 (_tmp61_);
								}
								_tmp77_ = item;
								gtk_widget_set_visible ((GtkWidget*) _tmp77_, TRUE);
								_g_object_unref0 (current_item);
							} else {
								SlingshotFrontendAppItem* _tmp78_ = NULL;
								_tmp78_ = item;
								gtk_widget_set_visible ((GtkWidget*) _tmp78_, FALSE);
							}
							_tmp79_ = item_iter;
							item_iter = _tmp79_ + 1;
							_g_object_unref0 (item);
						}
					}
				}
			}
		}
	}
	_tmp80_ = apps;
	slingshot_window_update_pages (self, _tmp80_);
	_tmp81_ = self->children;
	_tmp82_ = g_list_nth_data (_tmp81_, (guint) 0);
	gtk_widget_grab_focus ((GtkWidget*) ((SlingshotFrontendAppItem*) _tmp82_));
}


static void slingshot_window_change_category (SlingshotWindow* self) {
	GeeArrayList* _tmp0_ = NULL;
	SlingshotFrontendIndicators* _tmp1_ = NULL;
	gint _tmp2_ = 0;
	SlingshotFrontendIndicators* _tmp15_ = NULL;
	g_return_if_fail (self != NULL);
	_tmp0_ = self->filtered;
	gee_abstract_collection_clear ((GeeAbstractCollection*) _tmp0_);
	_tmp1_ = self->categories;
	_tmp2_ = _tmp1_->active;
	if (_tmp2_ != 0) {
		GeeArrayList* _tmp3_ = NULL;
		SlingshotFrontendIndicators* _tmp4_ = NULL;
		gint _tmp5_ = 0;
		gpointer _tmp6_ = NULL;
		GMenuTreeDirectory* _tmp7_ = NULL;
		GeeArrayList* _tmp8_ = NULL;
		GeeArrayList* _tmp9_ = NULL;
		GeeHashMap* _tmp10_ = NULL;
		gint _tmp11_ = 0;
		GeeArrayList* _tmp12_ = NULL;
		_tmp3_ = self->all_categories;
		_tmp4_ = self->categories;
		_tmp5_ = _tmp4_->active;
		_tmp6_ = gee_abstract_list_get ((GeeAbstractList*) _tmp3_, _tmp5_ - 1);
		_tmp7_ = (GMenuTreeDirectory*) _tmp6_;
		_tmp8_ = slingshot_backend_gmenu_entries_get_applications_for_category (_tmp7_);
		_tmp9_ = _tmp8_;
		_tmp10_ = self->icons;
		_tmp11_ = self->icon_size;
		slingshot_backend_gmenu_entries_enumerate_apps (_tmp9_, _tmp10_, _tmp11_, &_tmp12_);
		_g_object_unref0 (self->filtered);
		self->filtered = _tmp12_;
		_g_object_unref0 (_tmp9_);
		_gmenu_tree_item_unref0 (_tmp7_);
	} else {
		GeeArrayList* _tmp13_ = NULL;
		GeeArrayList* _tmp14_ = NULL;
		_tmp13_ = self->filtered;
		_tmp14_ = self->apps;
		gee_abstract_collection_add_all ((GeeAbstractCollection*) _tmp13_, (GeeCollection*) _tmp14_);
	}
	_tmp15_ = self->pages;
	slingshot_frontend_indicators_set_active (_tmp15_, 0);
}


static void slingshot_window_update_pages (SlingshotWindow* self, GeeArrayList* apps) {
	gint num_pages = 0;
	GeeArrayList* _tmp0_ = NULL;
	gint _tmp1_ = 0;
	gint _tmp2_ = 0;
	SlingshotFrontendGrid* _tmp3_ = NULL;
	guint _tmp4_ = 0U;
	guint _tmp5_ = 0U;
	SlingshotFrontendGrid* _tmp6_ = NULL;
	guint _tmp7_ = 0U;
	guint _tmp8_ = 0U;
	gint _tmp9_ = 0;
	GeeArrayList* _tmp10_ = NULL;
	gint _tmp11_ = 0;
	gint _tmp12_ = 0;
	SlingshotFrontendGrid* _tmp13_ = NULL;
	guint _tmp14_ = 0U;
	guint _tmp15_ = 0U;
	SlingshotFrontendGrid* _tmp16_ = NULL;
	guint _tmp17_ = 0U;
	guint _tmp18_ = 0U;
	gint _tmp23_ = 0;
	g_return_if_fail (self != NULL);
	g_return_if_fail (apps != NULL);
	_tmp0_ = apps;
	_tmp1_ = gee_abstract_collection_get_size ((GeeCollection*) _tmp0_);
	_tmp2_ = _tmp1_;
	_tmp3_ = self->grid;
	g_object_get ((GtkTable*) _tmp3_, "n-columns", &_tmp4_, NULL);
	_tmp5_ = _tmp4_;
	_tmp6_ = self->grid;
	g_object_get ((GtkTable*) _tmp6_, "n-rows", &_tmp7_, NULL);
	_tmp8_ = _tmp7_;
	num_pages = (gint) (_tmp2_ / (_tmp5_ * _tmp8_));
	_tmp10_ = apps;
	_tmp11_ = gee_abstract_collection_get_size ((GeeCollection*) _tmp10_);
	_tmp12_ = _tmp11_;
	_tmp13_ = self->grid;
	g_object_get ((GtkTable*) _tmp13_, "n-columns", &_tmp14_, NULL);
	_tmp15_ = _tmp14_;
	_tmp16_ = self->grid;
	g_object_get ((GtkTable*) _tmp16_, "n-rows", &_tmp17_, NULL);
	_tmp18_ = _tmp17_;
	if (fmod ((gdouble) _tmp12_, (gdouble) (_tmp15_ * _tmp18_)) > ((gdouble) 0)) {
		gint _tmp19_ = 0;
		gint _tmp20_ = 0;
		_tmp19_ = num_pages;
		self->total_pages = _tmp19_ + 1;
		_tmp20_ = self->total_pages;
		_tmp9_ = _tmp20_;
	} else {
		gint _tmp21_ = 0;
		gint _tmp22_ = 0;
		_tmp21_ = num_pages;
		self->total_pages = _tmp21_;
		_tmp22_ = self->total_pages;
		_tmp9_ = _tmp22_;
	}
	_tmp23_ = self->total_pages;
	if (_tmp23_ > 1) {
		SlingshotFrontendIndicators* _tmp24_ = NULL;
		_tmp24_ = self->pages;
		gtk_widget_set_visible ((GtkWidget*) _tmp24_, TRUE);
		{
			gint p = 0;
			p = 1;
			{
				gboolean _tmp25_ = FALSE;
				_tmp25_ = TRUE;
				while (TRUE) {
					gint _tmp27_ = 0;
					SlingshotFrontendIndicators* _tmp28_ = NULL;
					GList* _tmp29_ = NULL;
					guint _tmp30_ = 0U;
					gboolean _tmp31_ = FALSE;
					gint _tmp32_ = 0;
					gint _tmp33_ = 0;
					if (!_tmp25_) {
						gint _tmp26_ = 0;
						_tmp26_ = p;
						p = _tmp26_ + 1;
					}
					_tmp25_ = FALSE;
					_tmp27_ = p;
					_tmp28_ = self->pages;
					_tmp29_ = _tmp28_->children;
					_tmp30_ = g_list_length (_tmp29_);
					if (!(((guint) _tmp27_) <= _tmp30_)) {
						break;
					}
					_tmp32_ = p;
					_tmp33_ = self->total_pages;
					if (_tmp32_ > _tmp33_) {
						SlingshotFrontendIndicators* _tmp34_ = NULL;
						GList* _tmp35_ = NULL;
						gint _tmp36_ = 0;
						gconstpointer _tmp37_ = NULL;
						_tmp34_ = self->pages;
						_tmp35_ = _tmp34_->children;
						_tmp36_ = p;
						_tmp37_ = g_list_nth_data (_tmp35_, (guint) (_tmp36_ - 1));
						gtk_widget_set_visible ((GtkWidget*) _tmp37_, FALSE);
						_tmp31_ = FALSE;
					} else {
						SlingshotFrontendIndicators* _tmp38_ = NULL;
						GList* _tmp39_ = NULL;
						gint _tmp40_ = 0;
						gconstpointer _tmp41_ = NULL;
						_tmp38_ = self->pages;
						_tmp39_ = _tmp38_->children;
						_tmp40_ = p;
						_tmp41_ = g_list_nth_data (_tmp39_, (guint) (_tmp40_ - 1));
						gtk_widget_set_visible ((GtkWidget*) _tmp41_, TRUE);
						_tmp31_ = TRUE;
					}
				}
			}
		}
	} else {
		SlingshotFrontendIndicators* _tmp42_ = NULL;
		_tmp42_ = self->pages;
		gtk_widget_set_visible ((GtkWidget*) _tmp42_, FALSE);
	}
}


static gboolean string_contains (const gchar* self, const gchar* needle) {
	gboolean result = FALSE;
	const gchar* _tmp0_ = NULL;
	gchar* _tmp1_ = NULL;
	g_return_val_if_fail (self != NULL, FALSE);
	g_return_val_if_fail (needle != NULL, FALSE);
	_tmp0_ = needle;
	_tmp1_ = strstr ((gchar*) self, (gchar*) _tmp0_);
	result = _tmp1_ != NULL;
	return result;
}


static void slingshot_window_search (SlingshotWindow* self) {
	gchar* current_text = NULL;
	SlingshotFrontendSearchbar* _tmp0_ = NULL;
	gchar* _tmp1_ = NULL;
	gchar* _tmp2_ = NULL;
	gchar* _tmp3_ = NULL;
	gchar* _tmp4_ = NULL;
	gchar* _tmp5_ = NULL;
	SlingshotFrontendIndicators* _tmp6_ = NULL;
	GeeArrayList* _tmp7_ = NULL;
	SlingshotFrontendIndicators* _tmp45_ = NULL;
	g_return_if_fail (self != NULL);
	_tmp0_ = self->searchbar;
	_tmp1_ = slingshot_frontend_searchbar_get_text (_tmp0_);
	_tmp2_ = _tmp1_;
	_tmp3_ = _tmp2_;
	_tmp4_ = g_utf8_strdown (_tmp3_, (gssize) (-1));
	_tmp5_ = _tmp4_;
	_g_free0 (_tmp3_);
	current_text = _tmp5_;
	_tmp6_ = self->categories;
	slingshot_frontend_indicators_set_active_no_signal (_tmp6_, 0);
	_tmp7_ = self->filtered;
	gee_abstract_collection_clear ((GeeAbstractCollection*) _tmp7_);
	{
		GeeArrayList* _app_list = NULL;
		GeeArrayList* _tmp8_ = NULL;
		GeeArrayList* _tmp9_ = NULL;
		gint _app_size = 0;
		GeeArrayList* _tmp10_ = NULL;
		gint _tmp11_ = 0;
		gint _tmp12_ = 0;
		gint _app_index = 0;
		_tmp8_ = self->apps;
		_tmp9_ = _g_object_ref0 (_tmp8_);
		_app_list = _tmp9_;
		_tmp10_ = _app_list;
		_tmp11_ = gee_abstract_collection_get_size ((GeeCollection*) _tmp10_);
		_tmp12_ = _tmp11_;
		_app_size = _tmp12_;
		_app_index = -1;
		while (TRUE) {
			gint _tmp13_ = 0;
			gint _tmp14_ = 0;
			gint _tmp15_ = 0;
			GeeHashMap* app = NULL;
			GeeArrayList* _tmp16_ = NULL;
			gint _tmp17_ = 0;
			gpointer _tmp18_ = NULL;
			gboolean _tmp19_ = FALSE;
			gboolean _tmp20_ = FALSE;
			GeeHashMap* _tmp21_ = NULL;
			gpointer _tmp22_ = NULL;
			gchar* _tmp23_ = NULL;
			gchar* _tmp24_ = NULL;
			gchar* _tmp25_ = NULL;
			const gchar* _tmp26_ = NULL;
			gboolean _tmp27_ = FALSE;
			gboolean _tmp28_ = FALSE;
			_tmp13_ = _app_index;
			_app_index = _tmp13_ + 1;
			_tmp14_ = _app_index;
			_tmp15_ = _app_size;
			if (!(_tmp14_ < _tmp15_)) {
				break;
			}
			_tmp16_ = _app_list;
			_tmp17_ = _app_index;
			_tmp18_ = gee_abstract_list_get ((GeeAbstractList*) _tmp16_, _tmp17_);
			app = (GeeHashMap*) _tmp18_;
			_tmp21_ = app;
			_tmp22_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp21_, "name");
			_tmp23_ = (gchar*) _tmp22_;
			_tmp24_ = g_utf8_strdown (_tmp23_, (gssize) (-1));
			_tmp25_ = _tmp24_;
			_tmp26_ = current_text;
			_tmp27_ = string_contains (_tmp25_, _tmp26_);
			_tmp28_ = _tmp27_;
			_g_free0 (_tmp25_);
			_g_free0 (_tmp23_);
			if (_tmp28_) {
				_tmp20_ = TRUE;
			} else {
				GeeHashMap* _tmp29_ = NULL;
				gpointer _tmp30_ = NULL;
				gchar* _tmp31_ = NULL;
				gchar* _tmp32_ = NULL;
				gchar* _tmp33_ = NULL;
				const gchar* _tmp34_ = NULL;
				gboolean _tmp35_ = FALSE;
				_tmp29_ = app;
				_tmp30_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp29_, "description");
				_tmp31_ = (gchar*) _tmp30_;
				_tmp32_ = g_utf8_strdown (_tmp31_, (gssize) (-1));
				_tmp33_ = _tmp32_;
				_tmp34_ = current_text;
				_tmp35_ = string_contains (_tmp33_, _tmp34_);
				_tmp20_ = _tmp35_;
				_g_free0 (_tmp33_);
				_g_free0 (_tmp31_);
			}
			if (_tmp20_) {
				_tmp19_ = TRUE;
			} else {
				GeeHashMap* _tmp36_ = NULL;
				gpointer _tmp37_ = NULL;
				gchar* _tmp38_ = NULL;
				gchar* _tmp39_ = NULL;
				gchar* _tmp40_ = NULL;
				const gchar* _tmp41_ = NULL;
				gboolean _tmp42_ = FALSE;
				_tmp36_ = app;
				_tmp37_ = gee_abstract_map_get ((GeeAbstractMap*) _tmp36_, "command");
				_tmp38_ = (gchar*) _tmp37_;
				_tmp39_ = g_utf8_strdown (_tmp38_, (gssize) (-1));
				_tmp40_ = _tmp39_;
				_tmp41_ = current_text;
				_tmp42_ = string_contains (_tmp40_, _tmp41_);
				_tmp19_ = _tmp42_;
				_g_free0 (_tmp40_);
				_g_free0 (_tmp38_);
			}
			if (_tmp19_) {
				GeeArrayList* _tmp43_ = NULL;
				GeeHashMap* _tmp44_ = NULL;
				_tmp43_ = self->filtered;
				_tmp44_ = app;
				gee_abstract_collection_add ((GeeAbstractCollection*) _tmp43_, _tmp44_);
			}
			_g_object_unref0 (app);
		}
		_g_object_unref0 (_app_list);
	}
	_tmp45_ = self->pages;
	slingshot_frontend_indicators_set_active (_tmp45_, 0);
	gtk_widget_queue_draw ((GtkWidget*) self);
	_g_free0 (current_text);
}


static void slingshot_window_page_left (SlingshotWindow* self) {
	SlingshotFrontendIndicators* _tmp0_ = NULL;
	gint _tmp1_ = 0;
	g_return_if_fail (self != NULL);
	_tmp0_ = self->pages;
	_tmp1_ = _tmp0_->active;
	if (_tmp1_ >= 1) {
		SlingshotFrontendIndicators* _tmp2_ = NULL;
		SlingshotFrontendIndicators* _tmp3_ = NULL;
		gint _tmp4_ = 0;
		_tmp2_ = self->pages;
		_tmp3_ = self->pages;
		_tmp4_ = _tmp3_->active;
		slingshot_frontend_indicators_set_active (_tmp2_, _tmp4_ - 1);
	}
}


static void slingshot_window_page_right (SlingshotWindow* self) {
	SlingshotFrontendIndicators* _tmp0_ = NULL;
	gint _tmp1_ = 0;
	gint _tmp2_ = 0;
	g_return_if_fail (self != NULL);
	_tmp0_ = self->pages;
	_tmp1_ = _tmp0_->active;
	_tmp2_ = self->total_pages;
	if ((_tmp1_ + 1) < _tmp2_) {
		SlingshotFrontendIndicators* _tmp3_ = NULL;
		SlingshotFrontendIndicators* _tmp4_ = NULL;
		gint _tmp5_ = 0;
		_tmp3_ = self->pages;
		_tmp4_ = self->pages;
		_tmp5_ = _tmp4_->active;
		slingshot_frontend_indicators_set_active (_tmp3_, _tmp5_ + 1);
	}
}


static gboolean slingshot_window_draw_background (SlingshotWindow* self, GtkWidget* widget, GdkEventExpose* event) {
	gboolean result = FALSE;
	GtkAllocation size = {0};
	GtkWidget* _tmp0_ = NULL;
	GtkAllocation _tmp1_ = {0};
	cairo_t* context = NULL;
	GtkWidget* _tmp2_ = NULL;
	GdkWindow* _tmp3_ = NULL;
	cairo_t* _tmp4_ = NULL;
	cairo_pattern_t* linear_gradient = NULL;
	GtkAllocation _tmp5_ = {0};
	gint _tmp6_ = 0;
	GtkAllocation _tmp7_ = {0};
	gint _tmp8_ = 0;
	GtkAllocation _tmp9_ = {0};
	gint _tmp10_ = 0;
	GtkAllocation _tmp11_ = {0};
	gint _tmp12_ = 0;
	GtkAllocation _tmp13_ = {0};
	gint _tmp14_ = 0;
	cairo_pattern_t* _tmp15_ = NULL;
	g_return_val_if_fail (self != NULL, FALSE);
	g_return_val_if_fail (widget != NULL, FALSE);
	g_return_val_if_fail (event != NULL, FALSE);
	_tmp0_ = widget;
	gtk_widget_get_allocation (_tmp0_, &_tmp1_);
	size = _tmp1_;
	_tmp2_ = widget;
	_tmp3_ = _tmp2_->window;
	_tmp4_ = gdk_cairo_create ((GdkDrawable*) _tmp3_);
	context = _tmp4_;
	_tmp5_ = size;
	_tmp6_ = _tmp5_.x;
	_tmp7_ = size;
	_tmp8_ = _tmp7_.y;
	_tmp9_ = size;
	_tmp10_ = _tmp9_.x;
	_tmp11_ = size;
	_tmp12_ = _tmp11_.y;
	_tmp13_ = size;
	_tmp14_ = _tmp13_.height;
	_tmp15_ = cairo_pattern_create_linear ((gdouble) _tmp6_, (gdouble) _tmp8_, (gdouble) _tmp10_, (gdouble) (_tmp12_ + _tmp14_));
	linear_gradient = _tmp15_;
	cairo_pattern_add_color_stop_rgba (linear_gradient, 0.0, 0.0, 0.0, 0.0, 0.96);
	cairo_set_source (context, linear_gradient);
	cairo_paint (context);
	result = FALSE;
	_cairo_pattern_destroy0 (linear_gradient);
	_cairo_destroy0 (context);
	return result;
}


static gchar* string_slice (const gchar* self, glong start, glong end) {
	gchar* result = NULL;
	glong string_length = 0L;
	gint _tmp0_ = 0;
	gint _tmp1_ = 0;
	glong _tmp2_ = 0L;
	glong _tmp5_ = 0L;
	gboolean _tmp8_ = FALSE;
	glong _tmp9_ = 0L;
	gboolean _tmp12_ = FALSE;
	glong _tmp13_ = 0L;
	glong _tmp16_ = 0L;
	glong _tmp17_ = 0L;
	glong _tmp18_ = 0L;
	glong _tmp19_ = 0L;
	glong _tmp20_ = 0L;
	gchar* _tmp21_ = NULL;
	g_return_val_if_fail (self != NULL, NULL);
	_tmp0_ = strlen (self);
	_tmp1_ = _tmp0_;
	string_length = (glong) _tmp1_;
	_tmp2_ = start;
	if (_tmp2_ < ((glong) 0)) {
		glong _tmp3_ = 0L;
		glong _tmp4_ = 0L;
		_tmp3_ = string_length;
		_tmp4_ = start;
		start = _tmp3_ + _tmp4_;
	}
	_tmp5_ = end;
	if (_tmp5_ < ((glong) 0)) {
		glong _tmp6_ = 0L;
		glong _tmp7_ = 0L;
		_tmp6_ = string_length;
		_tmp7_ = end;
		end = _tmp6_ + _tmp7_;
	}
	_tmp9_ = start;
	if (_tmp9_ >= ((glong) 0)) {
		glong _tmp10_ = 0L;
		glong _tmp11_ = 0L;
		_tmp10_ = start;
		_tmp11_ = string_length;
		_tmp8_ = _tmp10_ <= _tmp11_;
	} else {
		_tmp8_ = FALSE;
	}
	g_return_val_if_fail (_tmp8_, NULL);
	_tmp13_ = end;
	if (_tmp13_ >= ((glong) 0)) {
		glong _tmp14_ = 0L;
		glong _tmp15_ = 0L;
		_tmp14_ = end;
		_tmp15_ = string_length;
		_tmp12_ = _tmp14_ <= _tmp15_;
	} else {
		_tmp12_ = FALSE;
	}
	g_return_val_if_fail (_tmp12_, NULL);
	_tmp16_ = start;
	_tmp17_ = end;
	g_return_val_if_fail (_tmp16_ <= _tmp17_, NULL);
	_tmp18_ = start;
	_tmp19_ = end;
	_tmp20_ = start;
	_tmp21_ = g_strndup (((gchar*) self) + _tmp18_, (gsize) (_tmp19_ - _tmp20_));
	result = _tmp21_;
	return result;
}


static gboolean slingshot_window_real_key_press_event (GtkWidget* base, GdkEventKey* event) {
	SlingshotWindow * self;
	gboolean result = FALSE;
	GdkEventKey _tmp0_ = {0};
	guint _tmp1_ = 0U;
	const gchar* _tmp2_ = NULL;
	const gchar* _tmp3_ = NULL;
	GQuark _tmp5_ = 0U;
	static GQuark _tmp4_label0 = 0;
	static GQuark _tmp4_label1 = 0;
	static GQuark _tmp4_label2 = 0;
	static GQuark _tmp4_label3 = 0;
	static GQuark _tmp4_label4 = 0;
	static GQuark _tmp4_label5 = 0;
	static GQuark _tmp4_label6 = 0;
	static GQuark _tmp4_label7 = 0;
	static GQuark _tmp4_label8 = 0;
	static GQuark _tmp4_label9 = 0;
	static GQuark _tmp4_label10 = 0;
	GdkEventKey _tmp57_ = {0};
	self = (SlingshotWindow*) base;
	g_return_val_if_fail (event != NULL, FALSE);
	_tmp0_ = *event;
	_tmp1_ = _tmp0_.keyval;
	_tmp2_ = gdk_keyval_name (_tmp1_);
	_tmp3_ = _tmp2_;
	_tmp5_ = (NULL == _tmp3_) ? 0 : g_quark_from_string (_tmp3_);
	if (_tmp5_ == ((0 != _tmp4_label0) ? _tmp4_label0 : (_tmp4_label0 = g_quark_from_static_string ("Escape")))) {
		switch (0) {
			default:
			{
				slingshot_window_destroy (self);
				result = TRUE;
				return result;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label1) ? _tmp4_label1 : (_tmp4_label1 = g_quark_from_static_string ("ISO_Left_Tab")))) {
		switch (0) {
			default:
			{
				slingshot_window_page_left (self);
				result = TRUE;
				return result;
			}
		}
	} else if ((_tmp5_ == ((0 != _tmp4_label2) ? _tmp4_label2 : (_tmp4_label2 = g_quark_from_static_string ("Shift_L")))) || (_tmp5_ == ((0 != _tmp4_label3) ? _tmp4_label3 : (_tmp4_label3 = g_quark_from_static_string ("Shift_R"))))) {
		switch (0) {
			default:
			{
				result = TRUE;
				return result;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label4) ? _tmp4_label4 : (_tmp4_label4 = g_quark_from_static_string ("Tab")))) {
		switch (0) {
			default:
			{
				slingshot_window_page_right (self);
				result = TRUE;
				return result;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label5) ? _tmp4_label5 : (_tmp4_label5 = g_quark_from_static_string ("Return")))) {
		switch (0) {
			default:
			{
				GeeArrayList* _tmp6_ = NULL;
				gint _tmp7_ = 0;
				gint _tmp8_ = 0;
				_tmp6_ = self->filtered;
				_tmp7_ = gee_abstract_collection_get_size ((GeeCollection*) _tmp6_);
				_tmp8_ = _tmp7_;
				if (_tmp8_ >= 1) {
					GtkWidget* _tmp9_ = NULL;
					GdkEventButton _tmp10_ = {0};
					gboolean _tmp11_ = FALSE;
					_tmp9_ = gtk_window_get_focus ((GtkWindow*) self);
					memset (&_tmp10_, 0, sizeof (GdkEventButton));
					g_signal_emit_by_name (_tmp9_, "button-release-event", &_tmp10_, &_tmp11_);
				}
				result = TRUE;
				return result;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label6) ? _tmp4_label6 : (_tmp4_label6 = g_quark_from_static_string ("BackSpace")))) {
		switch (0) {
			default:
			{
				SlingshotFrontendSearchbar* _tmp12_ = NULL;
				SlingshotFrontendSearchbar* _tmp13_ = NULL;
				gchar* _tmp14_ = NULL;
				gchar* _tmp15_ = NULL;
				gchar* _tmp16_ = NULL;
				SlingshotFrontendSearchbar* _tmp17_ = NULL;
				gchar* _tmp18_ = NULL;
				gchar* _tmp19_ = NULL;
				gchar* _tmp20_ = NULL;
				gint _tmp21_ = 0;
				gint _tmp22_ = 0;
				gchar* _tmp23_ = NULL;
				gchar* _tmp24_ = NULL;
				_tmp12_ = self->searchbar;
				_tmp13_ = self->searchbar;
				_tmp14_ = slingshot_frontend_searchbar_get_text (_tmp13_);
				_tmp15_ = _tmp14_;
				_tmp16_ = _tmp15_;
				_tmp17_ = self->searchbar;
				_tmp18_ = slingshot_frontend_searchbar_get_text (_tmp17_);
				_tmp19_ = _tmp18_;
				_tmp20_ = _tmp19_;
				_tmp21_ = strlen (_tmp20_);
				_tmp22_ = _tmp21_;
				_tmp23_ = string_slice (_tmp16_, (glong) 0, (glong) (((gint) _tmp22_) - 1));
				_tmp24_ = _tmp23_;
				slingshot_frontend_searchbar_set_text (_tmp12_, _tmp24_);
				_g_free0 (_tmp24_);
				_g_free0 (_tmp20_);
				_g_free0 (_tmp16_);
				result = TRUE;
				return result;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label7) ? _tmp4_label7 : (_tmp4_label7 = g_quark_from_static_string ("Left")))) {
		switch (0) {
			default:
			{
				gint current_item = 0;
				SlingshotFrontendGrid* _tmp25_ = NULL;
				GList* _tmp26_ = NULL;
				GList* _tmp27_ = NULL;
				GtkWidget* _tmp28_ = NULL;
				gint _tmp29_ = 0;
				gint _tmp30_ = 0;
				gint _tmp31_ = 0;
				SlingshotFrontendGrid* _tmp32_ = NULL;
				guint _tmp33_ = 0U;
				guint _tmp34_ = 0U;
				SlingshotFrontendGrid* _tmp35_ = NULL;
				guint _tmp36_ = 0U;
				guint _tmp37_ = 0U;
				_tmp25_ = self->grid;
				_tmp26_ = gtk_container_get_children ((GtkContainer*) _tmp25_);
				_tmp27_ = _tmp26_;
				_tmp28_ = gtk_window_get_focus ((GtkWindow*) self);
				_tmp29_ = g_list_index (_tmp27_, _tmp28_);
				_tmp30_ = _tmp29_;
				_g_list_free0 (_tmp27_);
				current_item = _tmp30_;
				_tmp31_ = current_item;
				_tmp32_ = self->grid;
				g_object_get ((GtkTable*) _tmp32_, "n-columns", &_tmp33_, NULL);
				_tmp34_ = _tmp33_;
				_tmp35_ = self->grid;
				g_object_get ((GtkTable*) _tmp35_, "n-columns", &_tmp36_, NULL);
				_tmp37_ = _tmp36_;
				if ((_tmp31_ % _tmp34_) == (_tmp37_ - 1)) {
					slingshot_window_page_left (self);
					result = TRUE;
					return result;
				}
				break;
			}
		}
	} else if (_tmp5_ == ((0 != _tmp4_label8) ? _tmp4_label8 : (_tmp4_label8 = g_quark_from_static_string ("Right")))) {
		switch (0) {
			default:
			{
				gint current_item = 0;
				SlingshotFrontendGrid* _tmp38_ = NULL;
				GList* _tmp39_ = NULL;
				GList* _tmp40_ = NULL;
				GtkWidget* _tmp41_ = NULL;
				gint _tmp42_ = 0;
				gint _tmp43_ = 0;
				gint _tmp44_ = 0;
				SlingshotFrontendGrid* _tmp45_ = NULL;
				guint _tmp46_ = 0U;
				guint _tmp47_ = 0U;
				_tmp38_ = self->grid;
				_tmp39_ = gtk_container_get_children ((GtkContainer*) _tmp38_);
				_tmp40_ = _tmp39_;
				_tmp41_ = gtk_window_get_focus ((GtkWindow*) self);
				_tmp42_ = g_list_index (_tmp40_, _tmp41_);
				_tmp43_ = _tmp42_;
				_g_list_free0 (_tmp40_);
				current_item = _tmp43_;
				_tmp44_ = current_item;
				_tmp45_ = self->grid;
				g_object_get ((GtkTable*) _tmp45_, "n-columns", &_tmp46_, NULL);
				_tmp47_ = _tmp46_;
				if ((_tmp44_ % _tmp47_) == ((guint) 0)) {
					slingshot_window_page_right (self);
					result = TRUE;
					return result;
				}
				break;
			}
		}
	} else if ((_tmp5_ == ((0 != _tmp4_label9) ? _tmp4_label9 : (_tmp4_label9 = g_quark_from_static_string ("Down")))) || (_tmp5_ == ((0 != _tmp4_label10) ? _tmp4_label10 : (_tmp4_label10 = g_quark_from_static_string ("Up"))))) {
		switch (0) {
			default:
			{
				break;
			}
		}
	} else {
		switch (0) {
			default:
			{
				SlingshotFrontendSearchbar* _tmp48_ = NULL;
				SlingshotFrontendSearchbar* _tmp49_ = NULL;
				gchar* _tmp50_ = NULL;
				gchar* _tmp51_ = NULL;
				gchar* _tmp52_ = NULL;
				GdkEventKey _tmp53_ = {0};
				const gchar* _tmp54_ = NULL;
				gchar* _tmp55_ = NULL;
				gchar* _tmp56_ = NULL;
				_tmp48_ = self->searchbar;
				_tmp49_ = self->searchbar;
				_tmp50_ = slingshot_frontend_searchbar_get_text (_tmp49_);
				_tmp51_ = _tmp50_;
				_tmp52_ = _tmp51_;
				_tmp53_ = *event;
				_tmp54_ = _tmp53_.string;
				_tmp55_ = g_strconcat (_tmp52_, _tmp54_, NULL);
				_tmp56_ = _tmp55_;
				slingshot_frontend_searchbar_set_text (_tmp48_, _tmp56_);
				_g_free0 (_tmp56_);
				_g_free0 (_tmp52_);
				break;
			}
		}
	}
	_tmp57_ = *event;
	GTK_WIDGET_CLASS (slingshot_window_parent_class)->key_press_event ((GtkWidget*) G_TYPE_CHECK_INSTANCE_CAST (self, ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, ElementaryWidgetsCompositedWindow), &_tmp57_);
	result = FALSE;
	return result;
}


static gboolean slingshot_window_real_scroll_event (GtkWidget* base, GdkEventScroll* event) {
	SlingshotWindow * self;
	gboolean result = FALSE;
	GdkEventScroll _tmp0_ = {0};
	GdkScrollDirection _tmp1_ = 0;
	GEnumValue* _tmp2_;
	const gchar* _tmp3_ = NULL;
	GQuark _tmp5_ = 0U;
	static GQuark _tmp4_label0 = 0;
	static GQuark _tmp4_label1 = 0;
	static GQuark _tmp4_label2 = 0;
	static GQuark _tmp4_label3 = 0;
	self = (SlingshotWindow*) base;
	g_return_val_if_fail (event != NULL, FALSE);
	_tmp0_ = *event;
	_tmp1_ = _tmp0_.direction;
	_tmp2_ = g_enum_get_value (g_type_class_ref (GDK_TYPE_SCROLL_DIRECTION), _tmp1_);
	_tmp3_ = (_tmp2_ != NULL) ? _tmp2_->value_name : NULL;
	_tmp5_ = (NULL == _tmp3_) ? 0 : g_quark_from_string (_tmp3_);
	if ((_tmp5_ == ((0 != _tmp4_label0) ? _tmp4_label0 : (_tmp4_label0 = g_quark_from_static_string ("GDK_SCROLL_UP")))) || (_tmp5_ == ((0 != _tmp4_label1) ? _tmp4_label1 : (_tmp4_label1 = g_quark_from_static_string ("GDK_SCROLL_LEFT"))))) {
		switch (0) {
			default:
			{
				slingshot_window_page_left (self);
				break;
			}
		}
	} else if ((_tmp5_ == ((0 != _tmp4_label2) ? _tmp4_label2 : (_tmp4_label2 = g_quark_from_static_string ("GDK_SCROLL_DOWN")))) || (_tmp5_ == ((0 != _tmp4_label3) ? _tmp4_label3 : (_tmp4_label3 = g_quark_from_static_string ("GDK_SCROLL_RIGHT"))))) {
		switch (0) {
			default:
			{
				slingshot_window_page_right (self);
				break;
			}
		}
	}
	result = FALSE;
	return result;
}


void slingshot_window_destroy (SlingshotWindow* self) {
	WnckScreen* _tmp0_ = NULL;
	g_return_if_fail (self != NULL);
	_tmp0_ = wnck_screen_get_default ();
	wnck_screen_toggle_showing_desktop (_tmp0_, FALSE);
	GTK_OBJECT_CLASS (slingshot_window_parent_class)->destroy ((GtkObject*) G_TYPE_CHECK_INSTANCE_CAST (self, ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, ElementaryWidgetsCompositedWindow));
	gtk_main_quit ();
}


static void slingshot_window_class_init (SlingshotWindowClass * klass) {
	slingshot_window_parent_class = g_type_class_peek_parent (klass);
	((GtkWidgetClass *) klass)->key_press_event = slingshot_window_real_key_press_event;
	((GtkWidgetClass *) klass)->scroll_event = slingshot_window_real_scroll_event;
	G_OBJECT_CLASS (klass)->finalize = slingshot_window_finalize;
}


static void slingshot_window_instance_init (SlingshotWindow * self) {
	GeeArrayList* _tmp0_ = NULL;
	GeeHashMap* _tmp1_ = NULL;
	GeeArrayList* _tmp2_ = NULL;
	GeeArrayList* _tmp3_ = NULL;
	self->children = NULL;
	_tmp0_ = gee_array_list_new (GEE_TYPE_HASH_MAP, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL);
	self->apps = _tmp0_;
	_tmp1_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, GDK_TYPE_PIXBUF, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL);
	self->icons = _tmp1_;
	_tmp2_ = gee_array_list_new (GEE_TYPE_HASH_MAP, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL);
	self->filtered = _tmp2_;
	_tmp3_ = slingshot_backend_gmenu_entries_get_categories ();
	self->all_categories = _tmp3_;
}


static void slingshot_window_finalize (GObject* obj) {
	SlingshotWindow * self;
	self = G_TYPE_CHECK_INSTANCE_CAST (obj, TYPE_SLINGSHOT_WINDOW, SlingshotWindow);
	__g_list_free__g_object_unref0_0 (self->children);
	_g_object_unref0 (self->searchbar);
	_g_object_unref0 (self->grid);
	_g_object_unref0 (self->apps);
	_g_object_unref0 (self->icons);
	_g_object_unref0 (self->filtered);
	_g_object_unref0 (self->pages);
	_g_object_unref0 (self->categories);
	_g_object_unref0 (self->all_categories);
	_g_object_unref0 (self->top_spacer);
	G_OBJECT_CLASS (slingshot_window_parent_class)->finalize (obj);
}


GType slingshot_window_get_type (void) {
	static volatile gsize slingshot_window_type_id__volatile = 0;
	if (g_once_init_enter (&slingshot_window_type_id__volatile)) {
		static const GTypeInfo g_define_type_info = { sizeof (SlingshotWindowClass), (GBaseInitFunc) NULL, (GBaseFinalizeFunc) NULL, (GClassInitFunc) slingshot_window_class_init, (GClassFinalizeFunc) NULL, NULL, sizeof (SlingshotWindow), 0, (GInstanceInitFunc) slingshot_window_instance_init, NULL };
		GType slingshot_window_type_id;
		slingshot_window_type_id = g_type_register_static (ELEMENTARY_WIDGETS_TYPE_COMPOSITED_WINDOW, "SlingshotWindow", &g_define_type_info, 0);
		g_once_init_leave (&slingshot_window_type_id__volatile, slingshot_window_type_id);
	}
	return slingshot_window_type_id__volatile;
}


gint _vala_main (gchar** args, int args_length1) {
	gint result = 0;
	UniqueApp* app = NULL;
	UniqueApp* _tmp0_ = NULL;
	UniqueApp* _tmp1_ = NULL;
	gboolean _tmp2_ = FALSE;
	gboolean _tmp3_ = FALSE;
	gtk_init (&args_length1, &args);
	_tmp0_ = unique_app_new ("org.elementary.slingshot", NULL);
	app = _tmp0_;
	_tmp1_ = app;
	g_object_get (_tmp1_, "is-running", &_tmp2_, NULL);
	_tmp3_ = _tmp2_;
	if (_tmp3_) {
		UniqueCommand command = 0;
		UniqueApp* _tmp4_ = NULL;
		UniqueCommand _tmp5_ = 0;
		UniqueMessageData* _tmp6_ = NULL;
		UniqueMessageData* _tmp7_ = NULL;
		command = UNIQUE_NEW;
		_tmp4_ = app;
		_tmp5_ = command;
		_tmp6_ = unique_message_data_new ();
		_tmp7_ = _tmp6_;
		unique_app_send_message (_tmp4_, (gint) _tmp5_, _tmp7_);
		_unique_message_data_free0 (_tmp7_);
	} else {
		SlingshotWindow* main_win = NULL;
		SlingshotWindow* _tmp8_ = NULL;
		SlingshotWindow* _tmp9_ = NULL;
		UniqueApp* _tmp10_ = NULL;
		SlingshotWindow* _tmp11_ = NULL;
		_tmp8_ = slingshot_window_new ();
		g_object_ref_sink (_tmp8_);
		main_win = _tmp8_;
		_tmp9_ = main_win;
		gtk_widget_show_all ((GtkWidget*) _tmp9_);
		_tmp10_ = app;
		_tmp11_ = main_win;
		unique_app_watch_window (_tmp10_, (GtkWindow*) _tmp11_);
		gtk_main ();
		_g_object_unref0 (main_win);
	}
	result = 1;
	_g_object_unref0 (app);
	return result;
}


int main (int argc, char ** argv) {
#if !GLIB_CHECK_VERSION (2,32,0)
	g_thread_init (NULL);
#endif
#if !GLIB_CHECK_VERSION (2,35,0)
	g_type_init ();
#endif
	return _vala_main (argv, argc);
}



